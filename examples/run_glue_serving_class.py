...
class PyTorchTransformer(object):
    ...
    def __init__(self, args):
        ...
        # Set seed
        PyTorchTransformer.set_seed(args)
    ...
    def run(args):
        ...
        # Training
        if args.do_train:
            train_dataset = PyTorchTransformer.load_and_cache_examples(args, args.task_name, tokenizer, evaluate=False)
            global_step, tr_loss = PyTorchTransformer.train(args, train_dataset, model, tokenizer)
        ...
        if args.do_eval and args.local_rank in [-1, 0]:
            ...
            for checkpoint in checkpoints:
                ...
                model.to(args.device)
                result = PyTorchTransformer.evaluate(args, model, tokenizer, prefix=global_step)
        ...
        if args.do_test and args.local_rank in [-1, 0] and args.test_file:
    ...
    def train(args, train_dataset, model, tokenizer):
        ...
        PyTorchTransformer.set_seed(args)  # Added here for reproductibility (even between python 2 and 3)
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                        ...
                        if args.local_rank == -1 and args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                            results = PyTorchTransformer.evaluate(args, model, tokenizer)
    ...
    def evaluate(args, model, tokenizer, prefix=""):
        ...
        for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
            eval_dataset = PyTorchTransformer.load_and_cache_examples(args, eval_task, tokenizer, evaluate=True)
    ...
    @staticmethod
    def test(dataframe, args, model, tokenizer, prefix=""):
        """
            refactor from evaluate() to assist for predicting
            Sharing the same params with additional dataframe input
        ...
        for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
            eval_dataset = PyTorchTransformer.load_predict_dataset(dataframe, args, eval_task, tokenizer, evaluate=True)
...
if __name__ == "__main__":
    args = PyTorchTransformer.get_params()
    PyTorchTransformer.run(args)
