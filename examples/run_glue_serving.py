...
def predict(dataframe, args, model, tokenizer, prefix=""):
    """
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        eval_dataset = load_predict_dataset(dataframe, args, eval_task, tokenizer, evaluate=True)
...
def main():
    ...
    if args.do_test and args.local_rank in [-1, 0] and args.test_file:
        ...
        for checkpoint in checkpoints:
            ...
            model = model_class.from_pretrained(checkpoint)
            model.to(args.device)
            preds = predict(dataframe, args, model, tokenizer, prefix=global_step)
            results.append(preds)
